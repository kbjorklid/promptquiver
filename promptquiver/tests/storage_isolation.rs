...
async fn test_storage_isolation_per_folder() {
    ...
    let note_a = Prompt::new("Note A".to_string(), PromptType::Note, Some(project_a.to_string()), None, None, None);
    storage.save_prompt(prompt_a.clone()).await.unwrap();
    storage.save_prompt(note_a.clone()).await.unwrap();

    // 2. Save global content
    let snippet_global = Prompt::new("Snippet Global".to_string(), PromptType::Snippet, None, None, Some("sg".to_string()), None);
