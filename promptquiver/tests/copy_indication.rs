...
use contracts::{Clipboard, Prompt, PromptType, Storage};
...
async fn test_copy_indication() {
    ...
    app.copy_selected().await.unwrap();
    assert!(
        !app.nav.prompts[0].last_copied,
        "First prompt should NO LONGER be marked as last_copied"
    );
...
async fn test_copy_via_y_key() {
    ...
    assert_eq!(
        current_clipboard, "Copy this text",
        "Clipboard should contain the prompt text after pressing 'y'"
    );
}
...
async fn test_copy_via_y_key_canned_tab() {
    ...
    let current_clipboard = clipboard.paste().await.unwrap();
    assert_eq!(
        current_clipboard, "Canned prompt text",
        "Clipboard should contain the canned prompt text after pressing 'y'"
    );
}
...
async fn test_copy_icon_rendering() {
    ...
    assert!(
        row_text.contains("📋"),
        "Rendered list should contain copy icon 📋. Found: {row_text}"
    );
}
