...
use ratatui::Frame;
use ratatui::layout::{Rect, Layout, Constraint, Direction, Alignment};
...
pub fn render_export_dialog(f: &mut Frame<'_>, state: &RenderState<'_, '_>) {
    ...
    f.render_widget(Paragraph::new(state.nav.data_manager.path.as_str()).block(path_block), chunks[1]);
    ...
    let cb_text = format!("{} Include Archived Prompts", if state.nav.data_manager.include_archived { "[x]" } else { "[ ]" });
    f.render_widget(Paragraph::new(cb_text).style(cb_style), chunks[3]);

    // Actions
    let action_text = "  [ Enter ] Export    [ Esc ] Cancel  ";
    f.render_widget(Paragraph::new(action_text).alignment(Alignment::Center).style(Style::default().fg(palette.muted)), chunks[4]);
}
...
pub fn render_import_dialog(f: &mut Frame<'_>, state: &RenderState<'_, '_>) {
    ...
    f.render_widget(Paragraph::new(ratatui::text::Text::from(text)).alignment(Alignment::Center), chunks[2]);
}
