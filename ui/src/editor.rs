...
pub fn render(f: &mut Frame<'_>, area: Rect, state: &mut RenderState<'_, '_>) -> Rect {
    ...
    textarea.set_block(
        Block::default()
            ...
            .border_type(ratatui::widgets::BorderType::Rounded)
            .title(main_title)
            .border_style(if !title_focused || !is_snippet {
                Style::default().fg(palette.accent)
            } else {
                Style::default().fg(palette.fg)
            })
    ...
    f.render_stateful_widget(
        scrollbar,
        editor_area.inner(ratatui::layout::Margin { vertical: 1, horizontal: 0 }),
        &mut scrollbar_state,
    );
...
pub fn render_autocomplete(f: &mut Frame<'_>, editor_area: Rect, state: &mut RenderState<'_, '_>) {
    let settings = state.settings;
    let palette = crate::utils::get_palette(settings.theme_name.as_deref());
    ...
    if autocomplete_open && !suggestions.is_empty() {
        ...
        let popup_width = 80;
        let popup_height_pref =
            (u16::try_from(suggestions.len()).unwrap_or(u16::MAX).saturating_add(2)).min(10);

        // Absolute screen coordinates of the cursor
        let cursor_x =
            editor_area.x.saturating_add(1).saturating_add(u16::try_from(col).unwrap_or(u16::MAX));
        let cursor_y =
            editor_area.y.saturating_add(1).saturating_add(u16::try_from(row).unwrap_or(u16::MAX));
        ...
        let items: Vec<ListItem<'_>> = suggestions
            ...
            .map(|(i, s)| {
                ...
                let display_text = if s.r#type == contracts::PromptType::Prompt
                    && !s.text.is_empty()
                    && s.text != *name
                {
                    format!("{name} - {}", s.text)
